...
describe("DeepSeekEndpoint", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(Secrets.Secrets.getAsync).mockImplementation((key) => {
      if (key === "DEEPSEEK_API_KEY") return Promise.resolve(testApiKey);
  ...
  describe("constructor", () => {
    it("should initialize with API key name", () => {
      const endpoint = new DeepSeekEndpoint("DEEPSEEK_API_KEY");
      expect(endpoint.apiKeyName).toBe("DEEPSEEK_API_KEY");
    });
  });
